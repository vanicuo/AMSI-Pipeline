function [weight,eta,Zs_opt,L_opt]=CMR_LCMV_Scalar_Beamformer(Lp,data, flags) %---------------------------------------------------------
...
Zs_opt=zeros(size(Lp,3),1);
L_opt =zeros(size(Lp,1),size(Lp,3));
if isfield(data,'eta')
    eta=data.eta;
else
...
%          L2 = Lp(:,:,i)*vvv(:,maxeig);
    ...
    weight(:,i)=InvRyyL*CMR_pinv(L'*InvRyyL);
...
%     end
    if(flags.wn)
        omega_temp=weight(:,i)'*weight(:,i);
        % real is needed for MATLAB acceleration --
        % potential complex output of sqrt operation disables acceleration!
        weight(:,i)=weight(:,i)/real(sqrt(omega_temp(1,1)));       
    end    
